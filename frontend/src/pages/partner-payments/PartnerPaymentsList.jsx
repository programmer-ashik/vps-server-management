...
function buildGroups(items) {
  ...
  for (const item of items) {
    const key = String(item.bankAccountNumber || item.partnerEmail || item.id).trim().toLowerCase();
    ...
    const itemSubmitted = new Date(item.submittedAt ?? item.createdAt ?? 0).getTime();
  ...
  return Array.from(groups.values())
    ...
    .sort(
      (a, b) => new Date(b.latestSubmittedAt ?? 0) - new Date(a.latestSubmittedAt ?? 0)
    );
}
...
export default function PartnerPaymentsList() {
  ...
  const [selectedGroupKey, setSelectedGroupKey] = useState(searchParams.get('bankAccount') ?? '');
  ...
  const { data = [], isLoading, isError } = useGetPartnerPaymentsQuery(queryParams);
  ...
  const filteredGroups = useMemo(() => {
    ...
    return groupedPayments.filter((group) => group.statusCounts[statusFilter] > 0);
  }, [groupedPayments, statusFilter]);

  const selectedGroup = useMemo(
    () => filteredGroups.find((group) => group.key === selectedGroupKey) ?? groupedPayments.find((group) => group.key === selectedGroupKey) ?? null,
  ...
  const modalRequests = useMemo(() => {
    ...
    return selectedGroup.requests.filter((request) => request.status === modalStatusFilter);
    ...
    <div className="max-w-7xl mx-auto">
      <div className="mb-6">
        ...
        <p className="mt-1 text-sm text-neutral-600 dark:text-neutral-400">
          Grouped by bank account so each partner keeps a single request history in one place.
        </p>
      </div>
          ...
          <tbody>
            {filteredGroups.length === 0 ? (
              ...
              filteredGroups.map((group) => (
                <tr key={group.key} className="border-b hover:bg-neutral-50 dark:hover:bg-neutral-900/50">
                  <td className="p-4">
                    <div className="font-medium">{group.partnerName}</div>
                    <div className="text-xs text-neutral-500">{group.partnerEmail}</div>
                  </td>
                  <td className="p-4 font-medium">{group.bankAccountNumber || '-'}</td>
                  <td className="p-4">
                    <div className="font-medium">{group.requests.length}</div>
                    <div className="text-xs text-neutral-500">
                      P {group.statusCounts.pending} / U {group.statusCounts.unpaid} / PD {group.statusCounts.paid}
                    </div>
                  </td>
      ...
      <Modal
        ...
        title={selectedGroup ? `${selectedGroup.partnerName} Requests` : 'Partner Requests'}
        size="xl"
      >
        {selectedGroup && (
          <div className="space-y-5">
            <div className="grid gap-4 md:grid-cols-4">
              <div className="rounded-lg border p-4">
                <div className="text-xs uppercase tracking-wide text-neutral-500">Bank Account</div>
                <div className="mt-2 font-medium">{selectedGroup.bankAccountNumber || '-'}</div>
              </div>
              <div className="rounded-lg border p-4">
                <div className="text-xs uppercase tracking-wide text-neutral-500">Requests</div>
                <div className="mt-2 text-2xl font-semibold">{selectedGroup.requests.length}</div>
              </div>
              <div className="rounded-lg border p-4">
                <div className="text-xs uppercase tracking-wide text-neutral-500">Total Amount</div>
                <div className="mt-2 text-2xl font-semibold">{formatUsd(selectedGroup.totalAmount)}</div>
              </div>
              <div className="rounded-lg border p-4">
                <div className="text-xs uppercase tracking-wide text-neutral-500">Latest Submission</div>
                <div className="mt-2 text-sm font-medium">{formatDate(selectedGroup.latestSubmittedAt)}</div>
              </div>
            </div>
            ...
            <div className="flex flex-wrap items-center gap-3">
              ...
              <div className="text-sm text-neutral-500">
                Showing {modalRequests.length} of {selectedGroup.requests.length} requests
              </div>
            </div>
